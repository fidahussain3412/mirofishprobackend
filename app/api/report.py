    ...
    try:
        ...
        def run_generate():
            try:
                ...
                if report.status == ReportStatus.COMPLETED:
                    ...
                    task_manager.fail_task(task_id, report.error or "报告生成Failed")
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "generating",
                "message": "报告生成任务已启动，请通过 /api/report/generate/status 查询Progress",
                "already_generated": False
            }
        })
        
    except Exception as e:
        logger.error(f"启动报告生成任务Failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
...
def get_report(report_id: str):
    ...
    try:
        ...
        logger.error(f"获取报告Failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def chat_with_report_agent():
    ...
    请求（JSON）：
        {
            "simulation_id": "sim_xxxx",        // 必填，模拟ID
            "message": "请解释一下舆情走向",    // 必填，用户Message
...
def get_report_progress(report_id: str):
    ...
    try:
        ...
        if not progress:
            return jsonify({
                "success": False,
                "error": f"报告不存在或Progress信息不可用: {report_id}"
            }), 404
        ...
        logger.error(f"获取报告ProgressFailed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
