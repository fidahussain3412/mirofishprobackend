"""
Report API routes
Provide simulation report generation, retrieval, conversation and other interfaces
"""
...
def get_report_progress(report_id: str):
    ...
    try:
        ...
        if not progress:
            return jsonify({
                "success": False,
                "error": f"Report does not exist or progress information unavailable: {report_id}"
            }), 404
...
def check_report_status(simulation_id: str):
    ...
    Used for frontend to determine if Interview feature is unlocked
...
def get_agent_log(report_id: str):
    """
    Get Report Agent's detailed execution logs
    
    Real-time acquisition of every step action during report generation, including:
    - Report start, planning start/completion
    - Start of each section, tool calls, LLM responses, completion
    - Report completion or failure
    
    Query parameters:
        from_line: From which line to start reading (optional, default 0, for incremental acquisition)
