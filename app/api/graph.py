...
def delete_project(project_id: str):
    ...
    if not success:
        return jsonify({
            "success": False,
            "error": f"项目不存在或删除Failed: {project_id}"
        }), 404
...
def generate_ontology():
    ...
    try:
        ...
        project.status = ProjectStatus.ONTOLOGY_GENERATED
        ProjectManager.save_project(project)
        logger.info(f"=== 本体生成完成 === Project ID: {project.project_id}")
    ...
    try:
        ...
        def build_task():
            ...
            try:
                ...
                # 更新项目状态为Failed
                build_logger.error(f"[{task_id}] 图谱构建Failed: {str(e)}")
        ...
        return jsonify({
            ...
            "data": {
                "project_id": project_id,
                "task_id": task_id,
                "message": "图谱构建任务已启动，请通过 /task/{task_id} 查询Progress"
            }
        })
