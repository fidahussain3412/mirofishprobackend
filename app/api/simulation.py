...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        if status in prepared_statuses and config_generated:
            ...
            # 如果状态是preparing但文件Completed，自动更New status为ready
            if status == "preparing":
                try:
                    ...
                    logger.warning(f"自动更New statusFailed: {e}")
            
            logger.info(f"模拟 {simulation_id} 检测Result: 已准备完成 (status={status}, config_generated={config_generated})")
            return True, {
                "status": status,
            ...
            logger.warning(f"模拟 {simulation_id} 检测Result: 未准备完成 (status={status}, config_generated={config_generated})")
            return False, {
...
def prepare_simulation():
    ...
    - Support force regenerate (force_regenerate=true)
    ...
    try:
        ...
        try:
            ...
            reader = ZepEntityReader()
            # Quickly read entities (no edge information needed, just count)
            filtered_preview = reader.filter_defined_entities(
                graph_id=state.graph_id,
                defined_entity_types=entity_types_list,
                enrich_with_edges=False  # Do not get edge information, speed up
            )
            ...
            # Failure does not affect subsequent process, background task will re-acquire
        ...
        # Update simulation status (including pre-acquired entity count)
...
def get_simulation(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"获取模拟状态Failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    try:
        ...
        # Sort by creation time in descending order，返回最新的
...
def get_simulation_history():
    ...
    用于首页历史项目展示，返回包含Project name、描述等丰富信息的模拟列表
    
    Query参数：
        limit: Return count limit（默认20）
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    实时获取模拟的Agent Profile（用于在生成过程中实时查看Progress）
...
def start_simulation():
    ...
    关于 force 参数：
        - 启用后，如果模拟正在运行或Completed，会先停止并清理运行日志
    ...
    try:
        ...
        logger.error(f"启动模拟Failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def stop_simulation():
    ...
    try:
        ...
        logger.error(f"停止模拟Failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_run_status(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"获取运行状态Failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_env_status():
    ...
    try:
        ...
        logger.error(f"获取环境状态Failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
