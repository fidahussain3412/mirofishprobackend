...
def prepare_simulation():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"同步获取实体数量Failed（将在后台任务中重试）: {e}")
        ...
        def run_prepare():
            try:
                ...
                def progress_callback(stage, progress, message, **kwargs):
                    ...
                    # 构建Detailed progress information
                    stage_names = {
                        "reading": "读取图谱实体",
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": "准备任务已启动，请通过 /api/simulation/prepare/status 查询Progress",
        ...
        logger.error(f"启动准备任务Failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_prepare_status():
    ...
    支持两种查询方式：
    1. 通过task_id查询正在进行的任务Progress
    2. 通过simulation_id检查是否已有完成的准备工作
    
    请求（JSON）：
        {
            "task_id": "task_xxxx",          // 可选，prepare返回的task_id
            "simulation_id": "sim_xxxx"      // 可选，模拟ID（用于检查Completed的准备）
        }
