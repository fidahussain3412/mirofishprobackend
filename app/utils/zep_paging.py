...
Zep node/edge list interface uses UUID cursor pagination,
This module encapsulates auto-pagination logic (including single-page retry), transparently returns complete list to caller.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single-page request, exponential backoff retry on failure. Only retry transient network/IO errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Get graph nodes via pagination, return at most max_items (default 2000). Each page request has built-in retry."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Get all graph edges via pagination, return complete list. Each page request has built-in retry."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
