...
【广度搜索 - 获取全貌视图】
这个工具用于获取模拟Result的完整全貌，特别适合了解事件演变过程。它会：
1. 获取所有相关节点和关系
...
【核心理念】
我们构建了一个模拟世界，并向其中注入了特定的「模拟需求」作为变量。模拟世界的演化Result，就是对未来可能发生情况的预测。你正在观察的不是"实验数据"，而是"未来的预演"。
...
系统会执行工具并把Result返回给你。你不需要也不能自己编写工具返回Result。
...
- 禁止自己编造工具返回Result（Observation），所有工具Result由系统注入
- 每次回复最多调用一个工具
...
4. 【引用格式规范 - 必须单独成段】
   ...
   ```
5. 保持与其他章节的逻辑连贯性
6. 【避免重复】仔细阅读下方Completed的章节内容，不要重复描述相同的信息
...
class ReportAgent:
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            ...
            logger.error(f"工具执行Failed: {tool_name}, 错误: {str(e)}")
            return f"工具执行Failed: {str(e)}"
    ...
    def _generate_section_react(
        ...
        # 构建用户prompt - 每个Completed章节各传入最大4000字
        ...
        if response is None:
            ...
            final_answer = f"（本章节生成Failed：LLM 返回空响应，请稍后重试）"
    ...
    def generate_report(
        ...
        try:
            ...
            logger.error(f"报告生成Failed: {str(e)}")
            ...
            try:
                ReportManager.save_report(report)
                ReportManager.update_progress(
                    report_id, "failed", -1, f"报告生成Failed: {str(e)}",
                    completed_sections=completed_section_titles
                )
            except Exception:
                pass  # 忽略保存Failed的错误
    ...
    def chat(
        ...
        try:
            ...
            logger.warning(f"获取报告内容Failed: {e}")
